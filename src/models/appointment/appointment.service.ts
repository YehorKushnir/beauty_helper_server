...
export class AppointmentService {
  ...
  async update(userId: string, id: string, dto: CreateUpdateAppointmentDto) {
    ...
    const paymentStatus = this.calculatePaymentStatus(prepared.totalAmount, appointment.payments)
  ...
  async schedule(userId: string, id: string) {
    ...
    const computedEndAt = appointment.endAt ?? new Date(appointment.startAt.getTime() + 60 * 1000)
  ...
  private async prepareItems(
    ...
    const createItems = normalizedItems.map((item) => {
      ...
      if (service.currency !== currency) {
        throw new BadRequestException('All services in one appointment must use same currency')
      }
